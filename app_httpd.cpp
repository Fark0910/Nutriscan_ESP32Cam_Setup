...
// =========================================================================================================================================================
//BARCODE IMAGE CAPTURE HANDLING, BARCODE READER API(CREATED BY US) TO FETCH EAN NUMBER AND EXPRESS.JS BASED BACKENED ROUTE CREATED BY US FOR EAN PROCESSING
// =========================================================================================================================================================
...
//=========================================================================================================================================================
void sendToNutriServer(const char* ean, const char* wifipass) {
  ...
  if (httpCode > 0) {
    Serial.printf("Nutri Server Response code: %d\n", httpCode);
    String payload = http.getString();
    Serial.println("Response payload:");
    Serial.println(payload);

//=========================================================================================================================================================
//CHECKING THE FLAG VALUE FROM JSON RESPONSE SEND BY "/nutri" if the product is safe turns on green led else turn on red
//=========================================================================================================================================================
...
// Access the flag inside esp.alerts[0].flag 
    if (!doc["esp"].isNull() && doc["esp"]["alerts"].is<JsonArray>()) {
      ...
      if (alerts.size() > 0 && alerts[0]["flag"]) {
        ...
        Serial.println("No alerts or flag missing.");
      }
    } 
    else {
...
//=========================================================================================================================================================
//Send captured image to barcode reader function 
//=========================================================================================================================================================
void sendCapturedImageToServer(uint8_t* imageData, size_t imageLen) {
  if (WiFi.status() != WL_CONNECTED) {
    Serial.println("WiFi not connected!");
    return;
  }
...
//=========================================================================================================================================================
  ...
  int contentLength = startRequest.length() + imageLen + endRequest.length();

//=========================================================================================================================================================
//Send http header to barcodedecoder.onrender.com/decode
//=========================================================================================================================================================
  client.println("POST /decode HTTP/1.1");
  client.println("Host: barcodedecoder.onrender.com");
  client.println("Content-Type: multipart/form-data; boundary=" + boundary);
  client.println("Content-Length: " + String(contentLength));
  client.println(); // Empty line to end headers

  // Send multipart body
  client.print(startRequest);  // Starting boundary
//=========================================================================================================================================================
//Image cannot be send directly send it in 512 bytes chunks esp32 might crash aur show guru mediation error trying to send at once due to space issue 
//at end pointi.e. /decode image bytes are merged and evaluated 
//=========================================================================================================================================================
  const size_t chunkSize = 512; // 512 bytes safe chunk
  size_t sent = 0;
  while (sent < imageLen) {
    size_t thisChunkSize = ((sent + chunkSize) > imageLen) ? (imageLen - sent) : chunkSize;
    client.write(&imageData[sent], thisChunkSize);
    sent += thisChunkSize;
    delay(1);  // slight yield to WiFi stack
  }

  client.print(endRequest);  // Ending boundary
...
//=========================================================================================================================================================
//Cleaning the response from /decode since it content unnecessary 0 after response 
//=========================================================================================================================================================
// Clean response before parsing
  int startIndex = response.indexOf('{');
  int endIndex = response.lastIndexOf('}');
  if (startIndex >= 0 && endIndex >= 0) {
    String cleanJson = response.substring(startIndex, endIndex + 1);
    
    StaticJsonDocument<300> doc;  // Important
    DeserializationError error = deserializeJson(doc, cleanJson);//important
    ...
    const char* ean = doc["barcodes"][0]["text"];
    Serial.print("EAN Number: ");
    Serial.println(ean);
//=========================================================================================================================================================
//Send to /nutri server defined in express.js both ean number and wifi pass this wifipass will be checked for given user in middleware of /nutri server
//=========================================================================================================================================================
    sendToNutriServer(ean,wifipass);  
...
#include "SPI.h"
//=========================================================================================================================================================
//Small tweaks in capture handler (capture button will be present on camserver route(UI) and this function handle it) 
//added RGB565 to jpeg conversion(very much important else your image will be null always)
//tested image saving in SD card(optional)
//=========================================================================================================================================================
...
//=========================================================================================================================================================
//CHECK FORMAT OF CAPTURED IMAGE FROM MY RHXY IT SUPPORT RGB
//=========================================================================================================================================================
  Serial.println(fb->format == PIXFORMAT_JPEG ? "JPEG format" : "RGB/YUV format");
  //=========================================================================================================================================================
...
//=========================================================================================================================================================
  ...
  /*
  // Save to SDCARD
  File file = SD_MMC.open("/capture.jpg", FILE_APPEND);// save as capture.jpg each time capture button is clicked this will overwrite
  if (!file) {
  ...
  file.close();*/
  Serial.printf("Saved JPEG image (%d bytes) to /capture.jpg\n", jpeg_len);
  //=========================================================================================================================================================
//Finally sending to sendCapturedImageToServer pass jpeg buff and jpeg length in params
//=========================================================================================================================================================
  sendCapturedImageToServer(jpeg_buf,jpeg_len);
  // Web response
  ...
  log_i("JPEG: %uB %ums", (uint32_t)(jpeg_len), (uint32_t)((fr_end - fr_start) / 1000));
